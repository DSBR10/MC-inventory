...
export default function BillingTagFilters({
  ...
  function applyFilter() {
    ...
    const newFilters = pendingValues.map((v) => ({
      key: pendingKey,
      value: v,
    }));
  ...
  function removeTag(tag: TagFilter) {
    onTagsChange(
      selectedTags.filter((t) => !(t.key === tag.key && t.value === tag.value)),
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)]/60 p-4 flex flex-wrap items-center gap-3">
      ...
      {Object.entries(groupedActive).map(([key, values]) => (
        ...
        >
          ...
          <span className="text-[var(--text-primary)]/40 text-xs">:</span>
            ...
            {values.map((v) => (
              ...
              >
                ...
                <button
                  ...
                  className="text-cyan-400 hover:text-[var(--text-primary)] transition-colors ml-0.5"
                >
                  ×
                </button>
              </span>
          ...
          <button
            ...
            className="text-[var(--text-primary)]/30 hover:text-red-400 transition-colors text-xs ml-1"
      ...
      <div className="relative" ref={ref}>
        <button
          onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/20 bg-black/30 text-sm text-[var(--text-primary)]/70 hover:text-[var(--text-primary)] hover:border-cyan-500/50 transition-all duration-200"
        >
          <svg
            className="w-4 h-4"
            fill="none"
        ...
        {open && (
          <div className="absolute left-0 top-full mt-2 z-50 bg-[#0d1117] border border-white/15 rounded-2xl shadow-2xl flex overflow-hidden min-w-[520px]">
            ...
            <div className="w-48 border-r border-[var(--border)] flex flex-col">
              <div className="px-4 py-3 text-xs font-semibold text-[var(--text-primary)]/40 uppercase tracking-widest border-b border-[var(--border)]">
                Tag Key
              </div>
              <div className="overflow-y-auto max-h-64">
                {allTagKeys.length === 0 ? (
                  <p className="px-4 py-3 text-xs text-[var(--text-primary)]/30">
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between ${
                        pendingKey === key
                          ...
                          : "text-[var(--text-primary)]/70 hover:bg-[var(--bg-card)]/60 hover:text-[var(--text-primary)]"
            ...
            <div className="flex-1 flex flex-col">
              <div className="px-4 py-3 text-xs font-semibold text-[var(--text-primary)]/40 uppercase tracking-widest border-b border-[var(--border)]">
                {pendingKey
                  ? `Valores de "${pendingKey}"`
                  : "Selecciona una key"}
              </div>

              <div className="overflow-y-auto max-h-64 flex-1">
                {!pendingKey ? (
                  <div className="flex items-center justify-center h-24 text-[var(--text-primary)]/20 text-sm">
                  ...
                  valuesForKey.map((val) => {
                    ...
                    return (
                      <label
                        key={val}
                        className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors hover:bg-[var(--bg-card)]/60 ${
                          checked
              ...
              {pendingKey && (
                <div className="border-t border-[var(--border)] px-4 py-3 flex items-center justify-between gap-2">
                  <span className="text-xs text-[var(--text-primary)]/30">
                    {pendingValues.length} seleccionado
                    {pendingValues.length !== 1 ? "s" : ""}
                  </span>
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 text-xs text-[var(--text-primary)]/40 hover:text-[var(--text-primary)] transition-colors"
                    >
                      Cancelar
                    </button>
                    <button
