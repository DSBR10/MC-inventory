...
import { normalizeInventoryItem } from "./normalize";
...
async function getAccountCCEInventory(account: HuaweiAccount) {
  try {
    ...
    if (!data || response?.status >= 400) {
      console.log(`[HUAWEI CCE] No data for account ${account.name}`);
      return [];
    }
    ...
    for (const cluster of clusters) {
      try {
        ...
        try {
          const nodesResponse = await huaweiRequest({
            method: "GET",

            host,

            uri: `/api/v3/projects/${account.projectId}/clusters/${clusterId}/nodes`,
          ...
          for (const node of nodes) {
            ...
            try {
              nodeTags = await getHuaweiTags({
                ...
                uri: `/api/v3/projects/${account.projectId}/clusters/${clusterId}/nodes/${node.metadata?.uid}/tags`,
              ...
              normalizeInventoryItem({
                ...
                status:
                  node.status?.phase === "Active"
                    ? "running"
                    : node.status?.phase?.toLowerCase() || "running", // Nunca UNKNOWN

                operatingSystem: node.spec?.os?.os || "Linux",
                ...
                publiclyExposed: !!node.status?.publicIP,

                internetFacing: !!node.status?.publicIP,
          ...
          console.warn(`CCE NODE ERROR ${cluster.metadata?.name}:`, err);
        }
        ...
        // 1. Intentar tags desde spec.clusterTags (ubicación correcta en Huawei CCE)
        if (clusterDetail.spec?.clusterTags) {
          for (const tag of clusterDetail.spec.clusterTags) {
        ...
        // 1.2. Fallback: Intentar tags directos metadata
        if (Object.keys(tags).length === 0 && cluster.metadata?.tags) {
        ...
        if (Object.keys(tags).length === 0) {
          try {
            ...
            const resourceTypes = ["cce", "cce-cluster", "clusters", "cluster"];
            ...
            for (const resourceType of resourceTypes) {
              try {
                ...
                const resources = tmsData?.data?.resources || [];
        ...
        inventory.push(
          normalizeInventoryItem({
            uniqueKey: `HUAWEI-${account.projectId}-CCE-${clusterId}`,
            ...
            status:
              cluster.status?.phase === "Available"
                ? "running"
                : cluster.status?.phase?.toLowerCase() || "running", // Nunca UNKNOWN
        ...
        console.error(`CCE DETAIL ERROR ${cluster.metadata?.name}:`, err);
      }
    }

    return inventory;
  } catch (error: any) {
    console.error("HUAWEI CCE ERROR:", error.message);
